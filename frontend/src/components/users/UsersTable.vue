        ...
        <tbody>
          <tr v-for="user in users" :key="user.id"
            ...
            <td class="px-4 py-3 flex gap-2">
              ...
              <button @click="$emit('change-password', user)"
              ...
              <button @click="$emit('manage-offices', user)"
                class="p-2 bg-green-600 hover:bg-green-700 text-white rounded transition" title="Офисы">
                🏢
              </button>
            </td>
          </tr>
          ...
          <tr v-if="loading">
            <td colspan="7" class="px-4 py-3 text-gray-500 text-center border-b border-gray-200">
              Загрузка...
            </td>
          </tr>

          <tr v-if="error">
            <td colspan="7" class="px-4 py-3 text-red-500 text-center border-b border-gray-200">
              {{ error }}
            </td>
          </tr>
        </tbody>
